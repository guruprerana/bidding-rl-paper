...
We implement our modular framework as a general-sum game between local policies seeking to fulfill their assigned objectives.
Each agent's action space is augmented with an additional numeric variable representing the bids, and the action from the highest bidder is played with uniform tie-breaking in case of draws.
...
We observe empirical convergence of training and substantially better or comparable performance to baselines in the three environments.
...
Lastly, to aid interpretability in renderings of rollouts, we implement visualization of the policy in control and the objective being pursued, using which we observe strategic bidding behaviors learned by the policies.
