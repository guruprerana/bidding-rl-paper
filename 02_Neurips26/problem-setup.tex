...
To capture stochastic target motion together with target appearance and disappearance, we formulate the urgency scores using conditional expectations under the transition kernel of the \momdp.
...
At a bidding round $(s,t)$, let $\Xi_{i}^{\win}(s,t)$ and $\Xi_{i}^{\lose}(s,t)$ denote the random future trajectories induced by the environment and by the subsequent bids when objective $i$ wins and loses the current auction, respectively.
...
\begin{theorem}
	...
	Suppose $\rho\beta > \max_{j\in [1;m]}\Delta_j^{\mathrm{gross}}(s,t)$, and let $i^*$ be a policy for which the following gross gain gap condition holds:
...
\begin{lemma}[Expiry-sensitive urgency gap]\label{lem:expiry urgency gap}
	...
	\geq \delta,
	\]
	and suppose that for every $j\neq i^*$, granting or denying the current bidding interval to $j$ does not change its probability of missing the deadline over the horizon, so that $\Delta_j^{\mathrm{gross}}(s,t)$ is due only to changes in discounted reward timing.
	Then
	\[
...
\begin{proof}
	...
	For every $j\neq i^*$, the no-expiry-change assumption implies that $\Delta_j^{\mathrm{gross}}(s,t)$ contains only the discounted reward timing difference, and therefore $\Delta_j^{\mathrm{gross}}(s,t)\leq r_{\max}$.
...
For the \allpay result below, we analyze the standard continuous-bid approximation of the auction game.
...
Writing $u_i(s,t)\coloneqq \Delta_i^{\mathrm{gross}}(s,t)$, define
\begin{align*}
...
\begin{assumption}[Urgency-completion bridge]
	...
	\]
	where $q_i(s,t)$ is the conditional probability that objective $i$ is eventually completed by horizon $T$ if it is granted control at $(s,t)$.
\end{assumption}
...
\begin{theorem}[Stochastic fraction-of-optimal completion guarantee]\label{thm:stochastic fraction optimal}
	Suppose the above three assumptions hold and $\beta(s,t)\geq \beta$ uniformly over reachable bidding states for some $\beta\in (0,1]$.
	Then
	\[
	\E[N^{\mathrm{auc}}(T)]
	\geq
