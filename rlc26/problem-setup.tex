...
Through this mechanism, policies scale their bids to reflect how critical their action is in the current state, and the bidding penalty incentivizes truthful urgency.
...
We formalize our new learning framework by modeling it as an $m$-player general-sum Markov game~\citep{littman1994markov}, where each player seeks to optimize one of the $m$ objectives of the original \momdp.
...
It follows from the construction that the optimal local policy of player $i$ in the Markov bidding game can be used as a bidding policy in the original \momdp.\GS{Not sure what this means.}
