...
An \momdp with \(m \in \N_{>0}\) objectives is specified by a tuple \(\mathcal{M} = (S, A, T, R, \mu_0)\) where \(S\) is the set of states, \(A\) is the set of actions, \(T : S \times A \to \dist(S)\) is the transition function mapping a state-action pair to a distribution over states, \(\mathbf{R}=\{R^i \colon S \times A^1\times\ldots\times A^m \times S \to \R_{\geq 0}\}_{i\in [1;m]}\) is the set of reward functions corresponding to the \(m\) objectives, and finally \(\mu_0 \in \dist(S)\) is the initial state distribution. 
...
In this work, we focus on maximizing the sum of rewards across the objectives. Concretely, we seek to find \(\arg\max_{\pi}J(\pi)\) where
\begin{equation}
    J(\pi) = \expectation_{(s_i, a_i) \sim \pi_{\mathcal{M}}}\left[ \sum_{t=0}^\infty \gamma^t \sum_{i = 1}^m R^i(s_i, a_i, s_{i+1})\right]
\end{equation}
