...
\contribution{
    % Contribution
    We present a novel, interpretable framework for designing adaptive RL policies for multi-objective problems in which objectives are drawn from the same family (here, cumulative discounted reward)\GS{Let's just go with reachability here}, they may appear or disappear during runtime, and an arbitrary number of them may be active at any time.
    }
    {
    % Caveat/Context:
    None
