...
Below, we show a connection between scheduling urgencies and the gross gains of policies.

\begin{lemma}\label{lem:critical regime gap}
	In both \poorman and \allpay settings, if the policy $i^*$ is in the critical regime, i.e., $\slack_{i^*}(s,t) \in [0,\tau]$, and every other policy $j\neq i^*$ is in the safe regime, i.e., $\slack_{j}(s,t)>\tau$, then 
	\begin{align}\label{eq:gross gain and urgency}
		\Delta_{i^*}^{\mathrm{gross}}(s,t) - \max_{j\neq i^*}\Delta_j^{\mathrm{gross}}(s,t) > M - r_{\max} > 0.
	\end{align}	
\end{lemma}

\begin{proof}	
	Since $\slack_{i^*}(s,t) < \tau$, if $i^*$ loses the bidding and assuming the highest bidding policy is not helping $i^*$ to reach its goal, by the next $\tau$ steps, it will have surely missed the deadline to reach its goal, and therefore $V_{i^*}^{\lose}(s,t,b_{i^*}) = -M - \rho b_{i^*}\cdot \mathbf{1}[\allpay]$.
	...
	Combining, $\Delta_{i^*}^{\mathrm{gross}}(s,t)-\max_{j\neq i^*}\Delta_j^{\mathrm{gross}}(s,t)  > M - r_j \geq M - r_{\max}$, which is the first inequality in \eqref{eq:gross gain and urgency}. 
	Finally, since we assumed that $M>r_{\max}$, the second inequality follows in \eqref{eq:gross gain and urgency}.
\end{proof}
