...
\textbf{Mobile cat feeder.} 
The environment is described in Section~\ref{sec:introduction} and is modeled using a $30\times 30$ grid with up to $m$ cats that are moving continuously according to unknown stochastic processes.
The value of $m$ was set as $3$ during the training time, whereas it was varied up to $8$ during the testing time.
...
To avoid sparsity of the reward function, we follow standard reward shaping techniques where the agent is also given rewards that are inversely proportional to its distance from the respective cat.
