...
when an objective appears or disappears, only the corresponding policy needs to be added or removed, without modifying the remaining policies. 
...
Another advantage of modularity is that it allows us to define objective-specific reward signals while the auction mechanism takes care of the trade-offs between objectives.
...
Even when the rewards can be aggregated into a single dense scalar signal, our experiments show that training such monolithic policies remains challenging, and their performance still falls short of that achieved by our modular approach.


Another advantage of our modular framework is interpretability.
At any moment, we can observe which policy is in control, and thereby which objective is currently being pursued by the agent. 
This kind of explanation is beneficial in a range of real-world situations, like providing post-hoc explanations of failures, and providing feedback on biases in pursuing different objectives.


We implement our modular framework as a nonzerosum game between agents that control the local policies to fulfill their assigned objectives, and this game can be set up using a simple wrapper around existing multi-objective environments. 
...
However, this introduces some new challenges, and below we explain them with our proposed remedies.
...
Additionally we train the policies against each other so that they learn to optimize payoffs in the presence of opponents.
