...
In the multi-objective setting, a commonly studied problem is to find a policy in the pareto-frontier of the value function which balances the tradeoffs between the objectives according to domain requirements~\citep{van2014multi}.
...
The bids are all nonnegative integers bounded by a given constant $\beta\in \mathbb{Z}_{\geq 0}$, and the highest bidder's actions get executed for the following $\tau$ consecutive steps, with bidding ties being resolved uniformly at random.
To discourage policies from always bidding high, we penalize the policies proportional to their bids with a given proportionality constant $\rho\in (0,1)$.
We consider two penalty models, namely \poorman and \allpay:
...
Consequently, \poorman encourages policies to bid when necessary, while \allpay discourages unnecessary bidding.
Through this mechanism, each policy can adjust its bid in proportion to the importance for it to execute its action in the current state, and the associated bidding penalty/reward aims to incentivize policies to bid in proportion to their true urgency.
Since one of the highest bidders obtains control, the policy with the greatest immediate importance is selected.
This way, we obtain a purely decentralized scheme to coordinate local policies in a given \momdp.

The parameters $\tau$, $\beta$, and $\rho$ will be called the \emph{bidding interval}, the \emph{maximum bid}, and the \emph{bid penalty coefficient}, respectively.
Through ablation studies in Section~\ref{sec:experimental results}, we demonstrate the importance of selecting these parameters appropriately.
If $\tau$ is too small, the switching between local policies could be too frequent, risking suboptimal outcome for all objectives.
If $\beta$ is too small, the policies will lose their ability to express the strength of their preferences, risking a random assignment of control in most cases.
Finally, if $\rho$ is too large, the policies will abstain from bidding in most situations.

\textbf{Local bidding policies via multiplayer Markov games.}
We formalize our new learning framework by modeling it as an $m$-player general-sum Markov game~\citep{littman1994markov}, where each player is concerned about on of the $m$ objectives of the original \momdp.
The transitions and the objectives in the game encodes the aforementioned bidding mechanism, and therefore, we call it a \emph{Markov bidding game}.
Each state of the game records the current state in the \momdp, the index of the policy selected from the last bidding, and the time until the next bidding.
Based on the current state in the game, each player independently selects its preferred action and its bid.
The resulting transitions encode the selection of one of the policies based on the bid value.
Finally, the reward of each player encodes the payment of the bidding penalties.
...
It follows from the construction that the optimal local policy of player $i$ in the Markov bidding game can be used as a bidding policy in the original \momdp.

\textbf{Learning policies in Markov bidding games.}
Since the transitions of the original \momdp is unknown, the transitions of the constructed Markov game in Definition~\ref{def:markov game} will also be unknown.
...
Each objective information vector is first processed by a multilayer perceptron (MLP) to produce a target embedding. 
