...
The policy $\pi$ gives rise to a random sequence of states and actions $s^0,a^0,s^1,a^1,\ldots$, that produces a random sequence of rewards $r^0_ir^1_i\ldots$ for each objective $i\in [1;m]$, where $r^t_i = R_i(s^t,a^t,a^{t+1})$ for every $t\in \mathbb{N}$.
The \textit{value} of $\pi$, denoted $V(\pi)$, is a vector in $\R^m$ such that $V(\pi)_i = \sum_{t=0}^\infty r^t_i$.
