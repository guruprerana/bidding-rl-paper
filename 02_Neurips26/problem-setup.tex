...
An \momdp with \(m \in \N_{>0}\) objectives is specified by a tuple \(\mathcal{M} = (S, A, T, \mu, R, \gamma)\) where 
\(S\) is the \textit{state space},
\(A\) is the set of \textit{action space}, 
\(T : S \times A \to \Delta(\mathbf{S})\) is the probabilistic \textit{transition function},
\(\mu \in \dist(S)\) is the \textit{initial state distribution},
\(R=\{r_i\colon S\times A\to \R_{>0}\}_{i\in [1;m]}\) is the set of \textit{reward functions} associated to the different objectives, and
$\gamma\in (0,1)$ is the \textit{discount factor}.
A (stationary) \textit{policy} in an \momdp~\(\mathcal{M}\) is a function \(\pi \colon S \to \dist(A)\) that maps the current state to a distribution over actions.
Executing the policy $\pi$ in $\M$ gives rise to a unique probability distribution $\mathbb{P}^\pi$ over sets of infinite sequences of states and actions $\rho = s^0a^0s^1a^1\ldots$, which we call a \textit{trace}. 
For every $i\in [1;m]$, the per-objective reward collected on the trace $\rho$ is $w_i(\rho)=\sum_{t=0}^\infty\gamma^{t}r_i(s^t,a^t)$.
The total reward collected on $\rho$ is: $w(\rho)=\sum_{i=1}^m w_i(\rho)$.
The \textit{value} of the policy $\pi$, denoted $V^\pi$, is given as $V^\pi = \E^\pi [w(\rho)]$.
...
\smallskip\noindent
\textbf{Lower complexity bound: simplified setting with deterministic agent and stationary goals.}\todo{this part needs work}
...
The bids are all non-negative rationals bounded by a given constant $\beta\in \mathbb{Q}_{\geq 0}$, and the highest bidder's actions get executed for the following $\tau$ consecutive steps, with bidding ties being resolved uniformly at random.
